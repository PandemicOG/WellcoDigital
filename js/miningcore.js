...
function loadBlocksPage() {
    return $.ajax(API + "pools/" + currentPool + "/blocks?page=0&pageSize=100")
        .done(function(data) {
            ...
            if (data.length > 0) {
                $.each(data, function(index, value) {
                    // Keep the date conversion for other potential uses
                    ...
                    var minerAddress = value.miner || 'Unknown';
                    var truncatedMiner = minerAddress.length > 10 ?
                        minerAddress.substring(0, 6) + '...' + minerAddress.substring(minerAddress.length - 4) :
                        minerAddress;

                    blockList += "<tr>";
                    blockList += `<td title="${minerAddress}">${truncatedMiner}</td>`;
...
function loadStatsChart() {
    return $.ajax(API + "pools/" + currentPool + "/performance")
        .done(function(data) {
            ...
            Chartist.Line("#chartStatsHashRate", {
                labels: labels,
                series: [networkHashRate]
            }, options);
            Chartist.Line("#chartStatsHashRatePool", {
                labels: labels,
                series: [poolHashRate]
            }, options);
            Chartist.Line("#chartStatsDiff", {
                labels: labels,
                series: [networkDifficulty]
            }, options);
            Chartist.Line("#chartStatsMiners", {
                labels: labels,
                series: [connectedMiners, connectedWorkers]
            }, options);
        })
        .fail(function() {
            $.notify({
...
function loadDashboardChart(walletAddress) {
    return $.ajax(API + "pools/" + currentPool + "/miners/" + walletAddress + "/performance")
        .done(function(data) {
            ...
            Chartist.Line("#chartDashboardHashRate", {
                labels: labels,
